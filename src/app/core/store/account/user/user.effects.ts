...
export class UserEffects {
    ...
    this.actions$.pipe(
      ...
      exhaustMap(credentials =>
        this.userService.signinUser(credentials).pipe(
          map(data => loginUserSuccess({ payload: data })),
          // tslint:disable-next-line:ban
          catchError(error =>
            of(
              error.headers.has('error-key')
                ? loginUserFail({ payload: { error: HttpErrorMapper.fromError(error) } })
                : generalError({ payload: { error: HttpErrorMapper.fromError(error) } })
            )
          )
        )
      )
    ...
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.userService.getCompanyUserData().pipe(
          map(user => loadCompanyUserSuccess({ payload: { user } })),
    ...
    this.actions$.pipe(
      ...
      mergeMap((data: CustomerRegistrationType) =>
        this.userService.createUser(data).pipe(
          ...
          map(() => loginUser({ payload: { credentials: data.credentials } })),
          // tslint:disable-next-line:ban
          catchError(error =>
            of(
              error.headers.has('error-key')
                ? createUserFail({ payload: { error: HttpErrorMapper.fromError(error) } })
                : generalError({ payload: { error: HttpErrorMapper.fromError(error) } })
            )
          )
        )
    ...
    this.actions$.pipe(
      ...
      concatMap(([payload, customer]) =>
        this.userService.updateUser({ user: payload.user, customer }).pipe(
          ...
          map(changedUser =>
            updateUserSuccess({ payload: { user: changedUser, successMessage: payload.successMessage } })
          ),
    ...
    this.actions$.pipe(
      ...
      concatMap(([payload]) =>
        this.userService.updateCustomer(payload.customer).pipe(
          ...
          map(changedCustomer =>
            updateCustomerSuccess({ payload: { customer: changedCustomer, successMessage: payload.successMessage } })
          ),
  ...
  loadUserByAPIToken$ = createEffect(() =>
    this.actions$.pipe(
      ...
      concatMap(apiToken =>
        this.userService.signinUserByToken(apiToken).pipe(map(user => loginUserSuccess({ payload: user })))
      )
    )
  );
    ...
    this.actions$.pipe(
      ...
      concatMap(([, customer]) =>
        this.paymentService.getUserPaymentMethods(customer).pipe(
          map(result => loadUserPaymentMethodsSuccess({ payload: { paymentMethods: result } })),
