...
export class ProductListingEffects {
  ...
  initializePageSize$ = createEffect(() =>
    this.actions$.pipe(take(1), mapTo(setProductListingPageSize({ payload: { itemsPerPage: this.itemsPerPage } })))
  );
  ...
  initializeDefaultViewType$ = createEffect(() =>
    this.store.pipe(
      ...
      whenFalsy(),
      mapTo(setViewType({ payload: { viewType: this.defaultViewType } }))
    )
  );
  ...
  setViewTypeFromQueryParam$ = createEffect(() =>
    this.store.pipe(
      ...
      map((viewType: ViewType) => setViewType({ payload: { viewType } }))
    )
  );
  ...
  determineParams$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(({ id, filters, sorting, page }) => loadMoreProductsForParams({ payload: { id, filters, sorting, page } }))
    )
  );
  ...
  loadMoreProducts$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(({ id, sorting, page, filters, viewAvailable }) => {
        if (viewAvailable) {
          return setProductListingPages({ payload: { id: { sorting, filters, ...id } } });
        }
        if (
          filters &&
          ...
          return loadProductsForFilter({ payload: { id: { ...id, filters }, searchParameter } });
        } else {
          switch (id.type) {
            case 'category':
              return loadProductsForCategory({ payload: { categoryId: id.value, page, sorting } });
            case 'search':
              return searchProducts({ payload: { searchTerm: id.value, page, sorting } });
            case 'master':
              return loadPagesForMaster({ payload: { id, sorting, filters } });
            default:
              return;
          }
        }
    ...
    this.actions$.pipe(
      ...
      map(({ type, value, filters }) => {
        if (
          ...
          const searchParameter = b64u.toBase64(b64u.encode(filters));
          return applyFilter({ payload: { searchParameter } });
        } else {
          switch (type) {
            case 'category':
              return loadFilterForCategory({ payload: { uniqueId: value } });
            case 'search':
              return loadFilterForSearch({ payload: { searchTerm: value } });
            case 'master':
              return loadPagesForMaster({ payload: { id: { type, value }, sorting: undefined, filters } });
            default:
              return;
          }
        }
      ...
      switchMap(({ id, filters }) =>
        this.store.pipe(
          ...
          mergeMap(product => {
            ...
            return [
              setProductListingPages({
                payload: this.productListingMapper.createPages(products, id.type, id.value, {
                  filters: filters ? b64u.toBase64(b64u.encode(filters)) : undefined,
                }),
              }),
              loadFilterSuccess({ payload: { filterNavigation } }),
            ];
          })
        )
      )
