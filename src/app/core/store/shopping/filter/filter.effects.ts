    ...
    this.actions$.pipe(
      ...
      mergeMap(uniqueId =>
        this.filterService.getFilterForCategory(uniqueId).pipe(
          map(filterNavigation => loadFilterSuccess({ payload: { filterNavigation } })),
    ...
    this.actions$.pipe(
      ...
      mergeMap(({ searchParameter }) =>
        this.filterService.applyFilter(searchParameter).pipe(
          map(availableFilter => applyFilterSuccess({ payload: { availableFilter, searchParameter } })),
        ...
        this.filterService.getFilteredProducts(searchParameter).pipe(
          mergeMap(({ productSKUs, total }) => [
            setProductListingPages({
              payload: this.productListingMapper.createPages(productSKUs, id.type, id.value, {
                filters: id.filters,
                itemCount: total,
              }),
            }),
